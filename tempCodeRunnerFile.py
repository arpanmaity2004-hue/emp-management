...
def get_current_employee():
    """
    Return currently logged-in employee.
    """
...
def admin_required(f):
    """
    Super Admin protection decorator.
    """
...
def hr_required(f):
    """
    HR and Super Admin protection decorator.
    """
...
def check_out():
    ...
    try:
        ...
        flash(
            f"Check-out recorded successfully. "
    ...
    if request.method == "POST":
        ...
        if days > leave_balance:
            ...
            flash(
                ...
                f"Available leave: {leave_balance} day(s).",
                "danger"
            )
...
def hr_dashboard():
    ...
    total_hr = Employee.query.filter_by(
        role="hr"
    ).count()

    present = sum(
        1
        for attendance, employee_obj in today_records
    ...
    pending = Leave.query.filter_by(
        status="Pending"
    ).count()

    approved = Leave.query.filter_by(
        status="Approved"
    ).count()

    rejected = Leave.query.filter_by(
        status="Rejected"
    ).count()
...
def update_leave(
    ...
    if action == "approve":
        ...
        # Check if employee still has enough balance
        ...
        if used_days + leave_obj.days > 12:
            ...
            flash(
                ...
                "does not have enough leave balance.",
                "danger"
            )
    ...
    try:
        ...
        flash(
            f"Leave {leave_obj.status.lower()} successfully.",
            "success"
        )
...
def admin_dashboard():
    ...
    total_admin = Employee.query.filter_by(
        role="admin"
    ).count()
...
@app.route(
    "/admin/leave/<int:leave_id>/<action>",
    methods=["POST"]
)
@admin_required
...
def add_employee():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(
                f"{role.upper()} account created successfully.",
                "success"
            )
...
@app.route(
    "/admin/change-role/<int:employee_id>/<new_role>",
    methods=["POST"]
)
@admin_required
def change_role(
    ...
    # HR role is permanent
    if target.role == "hr" and new_role == "employee":

        flash(
            f"{target.name} is already HR. "
            "HR cannot be changed back to Employee.",
            "warning"
        )
    ...
    if target.role == "employee" and new_role == "hr":
        ...
        try:
            ...
            flash(
                f"{target.name} has been promoted to HR. "
                "HR role is permanent.",
                "success"
            )
...
def init_app():
    ...
    with app.app_context():
        ...
        try:
            ...
            existing_admin = Employee.query.filter_by(
                email=admin_email
            ).first()
